...
/**
 * MyCakePHP_Sniffs_ControlStructures_SwitchDeclarationSniff.
 *
 ...
 * @license MIT
 */
class MyCakePHP_Sniffs_ControlStructures_SwitchDeclarationSniff implements PHP_CodeSniffer_Sniff {
	...
	public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr) {
		...
		while (($nextCase = $phpcsFile->findNext(array(
			T_CASE,
			T_DEFAULT,
			T_SWITCH), ($nextCase + 1), $switch['scope_closer'])) !== false) {
			...
			if ($tokens[$nextCase]['column'] !== $caseAlignment) {
				$error = strtoupper($type) . ' keyword must be indented one tab from SWITCH keyword';
			...
			if ($tokens[$nextBreak]['code'] === T_BREAK || $tokens[$nextBreak]['code'] === T_RETURN || $tokens[$nextBreak]['code'] === T_CONTINUE || $tokens[$nextBreak]['code']
				=== T_THROW) {
				if ($tokens[$nextBreak]['scope_condition'] === $nextCase) {
					...
					if ($tokens[$nextBreak]['column'] !== $caseAlignment + 1) {
						$error = 'Case breaking statement must be indented one tab from SWITCH keyword';
						$fix = $phpcsFile->addFixableError($error, $nextBreak, 'BreakIndent');

						if ($fix === true && $phpcsFile->fixer->enabled === true) {
							$padding = str_repeat("\t", ($caseAlignment));
		...
		if ($foundDefault === false) {
			$warning = 'All SWITCH statements should contain a DEFAULT case';
			$phpcsFile->addWarning($warning, $stackPtr, 'MissingDefault');
		}

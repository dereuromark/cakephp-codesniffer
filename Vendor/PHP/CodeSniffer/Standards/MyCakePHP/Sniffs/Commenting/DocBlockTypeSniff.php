...
/**
 * MyCakePHP_Sniffs_Commenting_DocBlockTypeSniff.
 *
 * @author Mark Scherer
 * @license MIT
 */
class MyCakePHP_Sniffs_Commenting_DocBlockTypeSniff implements PHP_CodeSniffer_Sniff {
    ...
    if (preg_match('/\@(\w+)\s+([\w\\|\\\\]*?)' . $from . '\b/i', $content, $matches) === 0) {
		...
		$error = 'Please use "' . $to . '" instead of "' . $from . '"';
		$phpcsFile->addFixableError($error, $stackPtr, 'WrongType');

		// Fix the error
		if ($phpcsFile->fixer->enabled === true) {
			$content = preg_replace('/\@(\w+)\s+([\w\\|\\\\]*?)' . $from . '\b/i', '@\1 \2' . $to, $content);
